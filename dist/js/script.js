...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function showTabContent(i = 0) {
    //Если функция вызывается без аргумента, она по умолчанию будет подставлять 0
  ...
  function getTimeRemaining(endtime) {
    //Задача ф-н получить разницу между данными
  ...
  modal.addEventListener('click', e => {
    //Закрытие модального окна по клику в серую область
    if (e.target === modal) {
      closeModal();
    }
  });
  ...
  const modalTimerId = setTimeout(openModal, 15000); //Если пользователь долистал до конца то мы показываем ему модальное окно
  ...
  new MenuCard("img/tabs/post.jpg", "post", 'Меню "Постное"', 'Меню "Постное" - это тщательный подбор ингредиентов: полное отсутствие продуктов животного происхождения, молоко из миндаля, овса, кокоса или гречки, правильное количество белков за счет тофу и импортных вегетарианских стейков.', 12.7, '.menu .container').render();
});
